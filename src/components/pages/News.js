...
function News({mode, setNavigate}) {

    const { data } = useFetch("https://old.bolalarolami.uz/uz/api/get-entity?slug=news&page=1")
    console.log(data ? data : "")
    const filteredData = data && data.latestNews.filter((item, i) => {
        return i > 1
    })
    setNavigate(false)
    return (
            ...
            {data && <div className='container'>
                <div className='news'>
                    <div className='news_hero'>
                        ...
                        <NavLink to={`../${data.itemList[1].id}`} className="hidden">
                            <div>
